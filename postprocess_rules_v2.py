"""
Post-procesador v2: Reglas 1 + 2 (ya validadas) + Regla 3 (SEXO invalido).
"""
...
# ── Reglas 1 y 2 (sin cambios) ───────────────────────────────────────────────
def aplicar_reglas_1_2(tokens, labels):
    labels = list(labels)
    # Regla 2: número atrapado en span SEXO
    ...
    # Regla 1: duplicado B-EDAD → B-DURACION si hay contexto de duración
...
# ── Extractor con Regla 3 integrada ──────────────────────────────────────────
def extraer(tokens, labels):
    res = {}

    # Regla 3: SEXO — iterar TODOS los spans SEXO, quedarse con el primero válido
    # (B-SEXO e I-SEXO, en orden de aparición)
    for tok, lbl in zip(tokens, labels):
        if "SEXO" in lbl:
            v = SEXO_MAP.get(norm(tok))
            if v is not None:
                res["sexo"] = v
                break   # primer valor válido encontrado → parar

    # Resto de campos (extractor estándar, solo B- tokens)
    for tok, lbl in zip(tokens, labels):
...
def inferir(texto):
    ...
    return extraer(tokens, labels_pp), extraer(tokens, labels_raw)
...
SET2 = [
    ("SOAP","Consulta medica. Paciente masculino de 45 anos de edad. Peso 82.0 kg, talla 170.5 cm. Motivo: dolor abdominal, diarrea, deshidratacion, 5 dias de evolucion. Signos vitales: presion arterial de 125/82 mmHg, glucosa de 98 mg/dl, temperatura 37.2°C, frecuencia cardiaca de 91 latidos por minuto.",{"edad":45,"sexo":"M","peso_kg":82.0,"talla_cm":170.5,"presion_sistolica":125,"presion_diastolica":82,"glucosa_mg_dl":98,"temperatura_c":37.2,"frecuencia_cardiaca_bpm":91,"duracion_sintomas_dias":5}),
    ("Dictado","Paciente femenina, 67 anos. PA: 160/95, glucemia capilar 310 mg/dL. T: 36.8 grados. FC: 88 lpm. Peso 63.5 kg, estatura de 155.0 cm. Sintomas: fatiga y perdida de peso desde hace 3 dias.",{"edad":67,"sexo":"F","peso_kg":63.5,"talla_cm":155.0,"presion_sistolica":160,"presion_diastolica":95,"glucosa_mg_dl":310,"temperatura_c":36.8,"frecuencia_cardiaca_bpm":88,"duracion_sintomas_dias":3}),
    ("Coloquial","Se atiende a mujer de 32 anos que lleva 7 dias con tos y dificultad para respirar. Presion 118 sobre 74, azucar en sangre 89, temperatura 38.5, pulso 102. Pesa 58 kilos, mide 162 cm.",{"edad":32,"sexo":"F","peso_kg":58.0,"talla_cm":162.0,"presion_sistolica":118,"presion_diastolica":74,"glucosa_mg_dl":89,"temperatura_c":38.5,"frecuencia_cardiaca_bpm":102,"duracion_sintomas_dias":7}),
    ("Pediatrico","Paciente masculino, edad: 2 anos. Consulta por fiebre leve postvacuna y control de esquema de vacunacion con 1 dia de evolucion. Peso corporal: 12.5 kg, talla 82.0 centimetros. Tension arterial 100/65 mmHg, temperatura de 37.8 grados, frecuencia cardiaca de 110 lpm, glucosa 88 mg/dl.",{"edad":2,"sexo":"M","peso_kg":12.5,"talla_cm":82.0,"presion_sistolica":100,"presion_diastolica":65,"glucosa_mg_dl":88,"temperatura_c":37.8,"frecuencia_cardiaca_bpm":110,"duracion_sintomas_dias":1}),
    ("TextoLibre","La senora tiene como 55 anos, es hipertensa, refiere zumbido en los oidos y vision borrosa desde hace 4 dias. Le tome la presion y salio 170 sobre 100. Su azucar estaba bien, 95. Temperatura normal 36.5. Pulso 78. Pesa como 70 kilos y mide 158 centimetros.",{"edad":55,"sexo":"F","peso_kg":70.0,"talla_cm":158.0,"presion_sistolica":170,"presion_diastolica":100,"glucosa_mg_dl":95,"temperatura_c":36.5,"frecuencia_cardiaca_bpm":78,"duracion_sintomas_dias":4}),
]
SET3 = [
...
# ── Evaluación ────────────────────────────────────────────────────────────────
def evaluar(nombre_set, casos):
    ok_sin = ok_con = total = 0
    cambios = []
    for nombre, texto, esp in casos:
        res_con, res_sin = inferir(texto)
        for campo, val_esp in esp.items():
            antes   = ok(res_sin.get(campo), val_esp, campo)
            despues = ok(res_con.get(campo), val_esp, campo)
            ok_sin += antes; ok_con += despues; total += 1
            if antes != despues:
                cambios.append({"caso":nombre,"campo":campo,"esp":val_esp,
                                 "sin":res_sin.get(campo),"con":res_con.get(campo),
                                 "tipo":"MEJORA" if despues and not antes else "REGRESION"})
    print(f"\n{'='*62}  {nombre_set}")
    print(f"  Sin reglas (R1+R2+R3): {ok_sin}/{total} ({ok_sin/total*100:.1f}%)")
    print(f"  Con reglas (R1+R2+R3): {ok_con}/{total} ({ok_con/total*100:.1f}%)")
    print(f"  Cambio neto: {'+' if ok_con>=ok_sin else ''}{ok_con-ok_sin} campos")
    if cambios:
        print(f"\n  Cambios:")
        for c in cambios:
            print(f"    [{c['tipo']:>9}] {c['caso']} | {c['campo']}: "
                  f"esp={c['esp']} sin={c['sin']} -> con={c['con']}")
    return ok_sin, ok_con, total, cambios

ok_s2_sin, ok_s2_con, t2, c2 = evaluar("SET 2 (5 textos, 50 campos)", SET2)
ok_s3_sin, ok_s3_con, t3, c3 = evaluar("SET 3 (20 ciegos, 189 campos)", SET3)

# ── Confirmaciones específicas: los 4 casos SEXO=None ─────────────────────────
CASOS_SEXO = [
    ("SET2/TextoLibre", "La senora tiene como 55 anos, es hipertensa, refiere zumbido en los oidos y vision borrosa desde hace 4 dias. Le tome la presion y salio 170 sobre 100. Su azucar estaba bien, 95. Temperatura normal 36.5. Pulso 78. Pesa como 70 kilos y mide 158 centimetros.", "F"),
    ("C12","Me llego este paciente femenino de 29 anos, con fiebre de 39.5 y llevaba 4 dias asi. La presion la tenia en 105/68. El pulso en 102.", "F"),
    ("C13","Datos del paciente: edad 58 anos, sexo masculino. Medidas: 74 kg / 166 cm. Signos vitales: tension 148/92 mmHg, glucemia 210 mg/dl, temperatura 37.0 grados, FC 88 lpm. Evolucion: 6 dias.", "M"),
    ("C14","Adulto mayor de sexo femenino, 78 anos. Refiere inicio hace 2 dias. Peso 52 kg. Talla 149 cm. La calentura: 37.8. Azucar en ayunas: 135. Tension: 168/102. Latidos: 92.", "F"),
]

print(f"\n{'='*62}")
print("  CONFIRMACION 1 — Los 4 casos con SEXO spurio (token invalido primero)")
print(f"{'='*62}")
print(f"  {'Caso':<20}  {'Antes(sin R3)':>14}  {'Despues(con R3)':>16}  {'Esp':>4}  OK?")
print("  " + "-"*60)
for nombre, texto, sexo_esp in CASOS_SEXO:
    tokens, labels_raw = raw_labels(texto)
    labels_pp = aplicar_reglas_1_2(tokens, labels_raw)
    # SIN Regla 3: toma primer B-SEXO aunque sea invalido
    sin_r3 = next((SEXO_MAP.get(norm(tok)) for tok,lbl in zip(tokens,labels_pp) if lbl=="B-SEXO"), None)
    # CON Regla 3: toma primer SEXO (B o I) con valor valido
    con_r3 = next((SEXO_MAP.get(norm(tok)) for tok,lbl in zip(tokens,labels_pp) if "SEXO" in lbl and SEXO_MAP.get(norm(tok))), None)
    correcto = "SI" if con_r3 == sexo_esp else "NO"
    print(f"  {nombre:<20}  {str(sin_r3):>14}  {str(con_r3):>16}  {sexo_esp:>4}  {correcto}")

print(f"\n{'='*62}")
print("  CONFIRMACION 2 — Resumen cambio vs. R1+R2 previamente validadas")
print(f"  (R1+R2 tenia: SET2=43/50, SET3=172/189)")
print(f"  Con R1+R2+R3: SET2={ok_s2_con}/50  SET3={ok_s3_con}/189")
reg = [c for c in c2+c3 if c["tipo"]=="REGRESION"]
print(f"  Regresiones: {len(reg)}")
for r in reg:
    print(f"    [{r['caso']}] {r['campo']}: esp={r['esp']} sin={r['sin']} -> con={r['con']}")
